...
func (s Settings) validate() error {
	if s.IntervalMinutes < 1 || s.IntervalMinutes > 1440 {
		return errors.New("فاصله ارسال باید بین ۱ تا ۱۴۴۰ دقیقه باشد")
	}
	if s.WorkerCount < 1 || s.WorkerCount > 20 {
		return errors.New("تعداد ورکر باید بین ۱ تا ۲۰ باشد")
	}
	if strings.TrimSpace(s.ChannelID) == "" {
		return errors.New("شناسه کانال الزامی است")
	}
	if s.MaxVideoMB < 1 || s.MaxVideoMB > 2000 {
		return errors.New("حداکثر حجم نامعتبر است")
	}
...
func loadStore(path string) (*Store, error) {
	...
	if len(b) > 0 && json.Unmarshal(b, s) != nil {
		return nil, errors.New("فایل داده خراب است")
	}
	if s.Sent == nil {
...
func (s *Store) wasSent(id string) bool {
	s.mu.RLock()
	db := s.db
	if db == nil {
		_, ok := s.Sent[id]
	...
	s.mu.RUnlock()
	var exists bool
	_ = db.QueryRowContext(context.Background(), "SELECT EXISTS (SELECT 1 FROM sent_videos WHERE id=$1)", id).Scan(&exists)
	return exists
}
func (s *Store) markSent(id string) error {
	...
	s.mu.RUnlock()
	if db != nil {
		_, e := db.ExecContext(context.Background(), "INSERT INTO sent_videos (id) VALUES ($1) ON CONFLICT (id) DO NOTHING", id)
		return e
	}
	s.mu.Lock()
	defer s.mu.Unlock()
...
func envInt(name string, fallback int) int {
	if n, e := strconv.Atoi(os.Getenv(name)); e == nil {
		return n
	}
	return fallback
}
