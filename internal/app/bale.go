...
type Bale struct{ client *http.Client; base, token string }
...
func (b *Bale) SendVideo(ctx context.Context, chat, path, caption string) error {
	...
	go func() {
		...
		if err := form.WriteField("chat_id", chat); err != nil { fail(err); return }
		if err := form.WriteField("caption", caption); err != nil { fail(err); return }
		...
		closeErr := file.Close()
		if copyErr != nil { fail(copyErr); return }
		if closeErr != nil { fail(closeErr); return }
		if err = form.Close(); err != nil { fail(err); return }
	...
	if err = <-writeErr; err != nil { return err }
	...
	if res.StatusCode/100 != 2 || !out.OK { return fmt.Errorf("خطای بله HTTP %d: %s", res.StatusCode, out.Description) }
	return nil
}
